...
spawned**. No AI, no pathfinding, no spawn caps, and bosses are on the menu. It gives Create's late
game something to spend SU on.

See [DESIGN.md](DESIGN.md) for the concept and [CHANGELOG.md](CHANGELOG.md) for what is in the mod.

## Status

`1.0.0`. The full loop is playable: bind a mob, train its model, simulate it for Predictions, stamp
them into drops, climb the substrate tiers, close the boss catalyst economy. Balance values live in
`simulacra-common.toml`.
...
Assets and data are hand-authored under `src/main/resources`. A `runData` config exists but
registers no `GatherDataEvent` providers, so it generates nothing. See [ROADMAP.md](ROADMAP.md).

The first build downloads NeoForge, Create, Flywheel, Ponder and Registrate, so it needs network
access and takes a while.
...
1. **Build a Cognition Array.** Neural Nodes are kinetic blocks with a shaft on the back. They draw
   ...
   every driven node it can reach. **Data Cables** extend the array to remote machines. One network,
   one controller. Extras go dormant.
...
5. **Spend them.** Feed Predictions to a **Loot Fabricator**. A funnel in, a funnel out. Left alone
   ...
   rotation, at `RPM / 8` work per tick, so a line sizes on paper: four nodes per chamber per
   Fabricator, and RPM/32 chambers between them.
...
chamber, Prediction and Fabricator chain follows theirs deliberately. New here is running it on
rotation: an array makes the compute that decides what you can simulate, and a shaft decides how
fast you stamp the results out.
